...
const Menu = () => {
  ...
  const [menuData, setMenuData] = useState(main_menu.filter((el) => el.category === "desserts"));

  // Состояние для модального окна
  const { isOpen, setIsOpen, setItemIndex } = useMenuModalStore();

  // Фильтрация кнопок для категорий
        ...
        <div className={scss.content}>
          <div className={scss.button_box}>
            {buttons.map((el, index) => (
              ...
              >
                <p className={`${scss.button_text} ${isActive === index ? scss.active : ""}`}>
                  {el}
                </p>
                {isActive === index && (
                  <motion.span
                ...
                {isActive === index && <span className={scss.button_line}></span>}
              </motion.button>
            ))}
          </div>
